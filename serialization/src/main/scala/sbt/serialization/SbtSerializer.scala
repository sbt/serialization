 ...
 * it be implicitly created when you define a sbt.serialization.Pickler and
 * sbt.serialization.Unpickler (or use the default definitions for those).
 */
final class SbtSerializer[T] private[serialization] (val pickler: Pickler[T], val unpickler: Unpickler[T])

object SbtSerializer {
  def apply[T](pickler: Pickler[T], unpickler: Unpickler[T]): SbtSerializer[T] =
...
trait SbtSerializers {
  ...
  implicit def sbtSerializerFromPicklerAndUnpickler[T](implicit pickler: Pickler[T], unpickler: Unpickler[T]): SbtSerializer[T] =
